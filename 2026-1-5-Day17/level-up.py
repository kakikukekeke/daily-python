...
def game():
  global Player_HP,Magic_point,Player_level,experience
  import random
###　Player attac patern

  Attac_patern = {
    "Blade":lambda:random.randint(2,4),
    "Magic":lambda:random.randint(2,3),
...
### Kind of Monster
  ...
  #get the dictionaly
    ...
    if Player_Attac in Attac_patern:
      ...
      if Monster_HP <= 0:
        Monster_HP = 0
        get_experience = random.randint(1,10)
        experience += get_experience
        print("get experience",get_experience)
        if experience >= 4:
          experience = 0
          Player_level += 1
          print("level UP !!!")
          print("player level is ",Player_level)
